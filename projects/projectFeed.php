...
if (($sortByIndex == "" || $sortByIndex == null) && $searchPageNumber != "") {
  ...
  if ($sortByIndex == 0) {
    ...
    $totalRowsResult = mysql_query("SELECT  * FROM  `PROJECT` WHERE `PUBLISH_STATUS`='now' AND `CATEGORY` LIKE '%$sortCategory%' OR '$sortCategory' LIKE '%CATEGORY%' ORDER BY `ID` ASC ", $connection);
    $totalRows       = mysql_num_rows($totalRowsResult);
    $startPage        = $totalRows - ($searchPageNumber) * 12;
    $count            = 12;
    if ($startPage < 0) {
    ...
    $query = "SELECT  * FROM  `PROJECT` WHERE `PUBLISH_STATUS`='now' AND `CATEGORY` LIKE '%$sortCategory%' OR '$sortCategory' LIKE '%CATEGORY%' ORDER BY `ID` ASC LIMIT " . $startPage . ", " . $count . "";
  }
}
