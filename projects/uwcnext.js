...
$(document).ready(function() {
  $("#createProjectButton").click(function(){
    $('index.php#create-project').tab('show');       
  });

  $("#myProjectButton").click(function(){    
    $('index.php#my-projects').tab('show'); 
  });
});
...
function helpInitiative(pageNumber) {
  ...
  $.ajax({
    ...
    if (projectData.length !== 0) {
      var foundSearch = ('<ul style= "float:right; font-size:12px; font-weight:bold; margin-top:8px;"><li style="display:inline; margin-left:10px;"> <select class="styled-select" id="sortByInitiative" onchange="Sort(this.value, 1)" style="font-size:12px; width:auto"><option value="" selected="selected">Sort By</option><option value ="current">Active Initiatives</option><option value="finished">Completed Initiatives</option><option value ="newest">Newest</option><option value="oldest">Oldest</option></select></li><li style="display:inline; margin-left:10px;"><form onsubmit="sortByLocation(); return false;" style="display:inline"><input id = "sortLocation" class="form-control" style="width:100px;  display:inline" type="search" name="sortLocation"  value = "" placeholder="Location"></form></li></ul>');
      ...
      for (var i = 0; i < projectData.length; i++) {
        ...
        for (var j = creatorData.length - 1; j >= 0; j--) {
          if (projectId == creatorData[j].PROJECT_ID) {
            ...
            creator = ($.trim(creator).length >=16) ? (creator).split(' ').slice(0, -1).join(' '): $.trim(creator);
      ...
      $("#totalProjectPagination").html("");
      $("#totalProjectPagination").html("");
      var searchPageCounter = (totalProjects % 12 == 0)? parseInt(totalProjects / 12) : parseInt(totalProjects / 12) + 1;
      if (searchPageCounter > 1) {
        ...
        if (pageNumber!=1){
          $("#totalProjectPagination").append('<li><a style="cursor:pointer" class="first" onclick="helpInitiative('+previous+')">Previous</a></li>');          
        }        
        for (var l = 1; l <= searchPageCounter; l++) {
          if (l ==pageNumber){
            $("#totalProjectPagination").append('<li><a style="cursor:pointer; background-color:#47a447; font-weight:600" onclick="helpInitiative(' + l + ')">' + l + '</a></li>');  
          } else {
            $("#totalProjectPagination").append('<li><a style="cursor:pointer" onclick="helpInitiative(' + l + ')">' + l + '</a></li>');
          }
        }
        if (pageNumber!=searchPageCounter){
          $("#totalProjectPagination").append('<li><a style="cursor:pointer" class="last" onclick="helpInitiative(' + next + ')">Next</a></li>');
        }
    ...
    if (projectData.length !== 0) {
      ...
      for (var i = 0; i < projectData.length; i++) {
        ...
        if (uid == "" || uid == null) {
          ...
          var column = '<li style="list-style-type:none; margin-bottom:35px" class="span3 tm_curved-vt-2"><div class="tm_text-shadow"><div class="thumbnail thumbnail-1"><img onclick="showDescription(' + projectId + ',10)" src="uploads/' + image + '" alt="" style="cursor:pointer; width:270px; height:192px; margin-bottom:20px"><section> <div class="mid-right-box" style="vertical-align:bottom"><h5 style="min-height:50px"><a onclick="showDescription(' + projectId + ',10)">' + projectName + '</a><div>' + videoIcon + '</div></h5></div><div class="meta"><div class="name-author"><i class="icon-user"></i> <a onclick="showUserProfile(\'' + creatorEmail + '\')">' + creator + '</a></div><div class="uwc-name"><i class="icon-certificate"></i><a class="uwcN">'+creatorUWC + " "+'</a><a class="uwcY">'+creatorUWCYear +'</a></div></div><div class="clear"></div> <p id="' + idForBlurb + '" style="min-height:110px; text-align:left; font-size:12px; margin-bottom:-25px;">' + projectBlurb + '</p><div style="margin-bottom:20px;"><a class="comments"><i class="icon-map-marker"></i> <b>' + location + '' + country + '</b></a></div><div class="progress"><div data-percentage="0%" style="width: ' + progreePercentage + '%;" class="progress-bar progress-bar-success" role="progressbar" aria-valuemin="0" aria-valuemax="100"></div></div><div style="padding-bottom:35px;" ><a style="color:white; background:rgb(87, 168, 62);" class="btn pull-right " onClick="actionForm(' + projectId + ',0)">Follow ' + follower + '</a> </div></section></div></div></li>';
        }
        if (publishStatus == "now") { 
...
function sortCategory(event, pageNumber) {
  var chosenCategory = (isNaN(event.target.text)) ? event.target.text : event.target.id;
  $("#feed").html("");
  $("#totalProjectPagination").html("");
  $("#title").html(chosenCategory);
  $.ajax({
    type: "POST",
    url: "projectFeed.php",
    data: {
    ...
    if (projectData.length !== 0) {
      ...
      if (uid == "" || uid == null) {
        var column = '<li style="list-style-type:none; margin-bottom:35px"><div><section><div><h6 style="text-align:center;">Sorry, there are no projects in this category.  <a href="../login_page.php" class="btn btn-1" style="color:white; background:rgb(87, 168, 62);">Create Your Own</a></h6></div></section></div></li>';
      } else {
        var column = '<li style="list-style-type:none; margin-bottom:35px"><div><section><div><h6 style="text-align:center;">Sorry, there are no projects in this category.  <a href="#tab4" data-toggle="tab" class="btn btn-1" style="color:white; background:rgb(87, 168, 62);">Create Your Own</a></h6></div></section></div></li>';
      }
      $('#feed').append(column);
    }
    $("#totalProjectPagination").html("");
    var searchPageCounter = (totalProjects % 12 == 0)? parseInt(totalProjects / 12) : parseInt(totalProjects / 12) + 1;
    if (searchPageCounter > 1) {
      // $("#totalProjectPagination").append('<li><a class="first" id="'+chosenCategory+'"  onclick="sortCategory(event, 1)">First</a></li>');
      for (var l = 1; l <= searchPageCounter; l++) {
        $("#totalProjectPagination").append('<li><a id="'+chosenCategory+'" onclick="sortCategory(event, '+l+')">' + l + '</a></li>');
      }
      // $("#totalProjectPagination").append('<li><a class="last" id="'+chosenCategory+'" onclick="sortCategory( event, ' + searchPageCounter + ')">Last</a></li>');
    }
  });
}
    ...
    $.ajax({
      ...
      if (projectData.length !== 0) {
        ...
        */
        ...
        if (uid == "" || uid == null) {
          var column = '<li style="list-style-type:none; margin-bottom:35px"><div><section><div><h6 style="text-align:center;">Sorry, there are no projects from '+$sortLocation+'.  <a href="../login_page.php" class="btn btn-1" style="color:white; background:rgb(87, 168, 62);">Create Your Own</a></h6></div></section></div></li>';
          } else {
            var column = '<li style="list-style-type:none; margin-bottom:35px"><div><section><div><h6 style="text-align:center;">Sorry, there are no projects from '+$sortLocation+'.  <a href="#tab4" data-toggle="tab" class="btn btn-1" style="color:white; background:rgb(87, 168, 62);">Create Your Own</a></h6></div></section></div></li>';
          }
        $('#feed').append(column);
      }
    });
