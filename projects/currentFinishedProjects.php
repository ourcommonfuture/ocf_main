...
if ($tag == 'finished') {
  $data      = mysql_fetch_object(mysql_query("SELECT COUNT(id) AS num_rows FROM `PROJECT` WHERE `PUBLISH_STATUS`='now' AND `PROGRESS_PERCENTAGE` >= 100"));
  ...
  $query = "SELECT  * FROM  `PROJECT`  WHERE `PUBLISH_STATUS`='now' AND `PROGRESS_PERCENTAGE` >= 100 ORDER BY `ID` ASC LIMIT " . $startPage . ", " . $count . " ";
} else {
  $data      = mysql_fetch_object(mysql_query("SELECT COUNT(id) AS num_rows FROM `PROJECT` WHERE `PUBLISH_STATUS`='now' AND `PROGRESS_PERCENTAGE` < 100"));
  $totalRows = $data->num_rows;
  $startPage = $totalRows - ($searchPageNumber) * 12;
