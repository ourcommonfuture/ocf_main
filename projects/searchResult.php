...
<?php
	...
	if ($search_input != ''){
		...
		$data = mysql_fetch_object(mysql_query("SELECT  COUNT(id) AS num_rows FROM  `PROJECT` WHERE `PUBLISH_STATUS`='now' AND `NAME` LIKE '%$search_input%' OR `LOCATION` LIKE '%$search_input%'
		...
		if ($totalRows > 0) {
			...
			// 	$startPage = 0;
			// }
			$query = "SELECT  * FROM  `PROJECT` WHERE `PUBLISH_STATUS`='now' AND  `NAME` LIKE '%$search_input%' OR `LOCATION` LIKE '%$search_input%'
